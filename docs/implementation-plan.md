...
- [x] T5.1 e2e-happy · - [ ] T5.2 e2e-edge · - [ ] T5.3 final-gate
...
- **F-6 (from T5.1, REAL customer-visible bug — fix before final gate):** the decision message renders the mandatory disclaimer TWICE — once mangled mid-paragraph in guillemets (`«...»`), once correctly as the small-print element. Root cause: `lib/ai/prompts.ts` includes the disclaimer inside a guillemet-quoted instruction that the model echoes literally into `messageMarkdown`; then `lib/ai/guard.ts` `ensureDisclaimer` only strips/detects an EXACT trailing match, so it doesn't recognize the guillemet-wrapped copy and appends a second clean one. Fix (be): stop embedding the literal disclaimer text in the prompt as quotable copy (instruct the model NOT to write it — the guard owns it), and/or make `ensureDisclaimer` detect an existing disclaimer anywhere in the body (normalize/strip before appending). Add a unit test. Owner: be (prompts.ts + guard.ts).
- **F-7 (observation from T5.1, not a bug — prompt tuning):** the real vision+decision model returned `Odrzucono` (REJECT) for ALL three photo scenarios including the clean-product return, citing R-5/R-12 "visible signs of usage". Tests tolerate this (structure-only). But a demo that rejects every case reads poorly. Consider prompt tuning so a genuinely clean/undamaged in-window product trends APPROVE/MORE_INFO. Investigate during T5.2 (which asserts specific categories for edge cases and will expose whether the guard/prompt is over-conservative) or a dedicated prompt-review micro-task. Not gating.
