...
- **F-1 (from T4.1) — FIXED (merged 8766315):** `requestFormSchema` now uses Zod v4 `refine(..., { when })` scoped to `requestType`+`reason`, so the AC-03 "reason required for complaint" error fires independently of sibling-field parse errors. Client/server identical (TAC-002-01). Regression test added.
