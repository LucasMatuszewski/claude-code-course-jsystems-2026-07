...
export const JSXPreviewContent = memo(
  ({ className, ...props }: JSXPreviewContentProps) => {
    ...
    useEffect(() => {
      ...
      // Vendor pattern from the ai-elements CLI: resets error state when
      // the streamed jsx input changes, not a state-mirroring anti-pattern.
    ...
    // During streaming, if the current JSX errored, re-render with last good
    // version. Vendor pattern from the ai-elements CLI: reads the
    // last-known-good JSX ref; the ref is only ever written in effects.
