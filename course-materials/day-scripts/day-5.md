...
- Brief model/provider landscape: Claude aliases `fable` (Fable 5, most capable), `opus` (Opus 4.8), `sonnet` (Sonnet 5), `haiku` (Haiku 4.5), plus local/open models (GLM, Qwen, Llama via Ollama)
...
**Demo:** Show Ollama running locally, Claude Code-first local/open-model workflow, or GLM Coding Plan.
